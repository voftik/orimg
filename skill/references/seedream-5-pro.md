...
- **Layer separation**: 5.0 Pro can natively return a composition split into transparent-background layers, but OpenRouter's `/images` endpoint exposes no field for it and orimg returns single flattened images — do NOT promise layered output. If a layered look is needed, generate elements as separate jobs (one per layer) and let the user composite them.
- **Web grounding**: 5.0 can consult web search before rendering. There is NO `web_search` field on OpenRouter's `/images` endpoint (orimg rejects unknown fields) — trigger retrieval through prompt text by naming real entities, recent events or date ranges: "Search for [data/event] from [date range], then generate a poster in [style] visualizing: 1) [dimension] 2) [dimension]." Always proofread rendered numbers. Skip for fiction: it adds cost and latency.
- **Batch series**: the native Ark `max_images`/sequential parameter is NOT exposed through OpenRouter (n caps at 1 for Seedream, and orimg rejects unknown fields) — get a series by duplicating jobs with enumerated prompts: "Image 1 of 4: the same sneaker in white/blue colorway. Identical angle, studio lighting." One job per variant.
